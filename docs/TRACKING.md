...
车前/车后判断现在先将 odom 轨迹转换到当前车体系，避免车辆平移或转向后错误选择前方杂波。
雷达更新将目标移出相机视野时，清除旧的“视野内未检测到”计时；目标仍在视野内时保留证伪机制。
`lidar_handoff_max_s` 限制的是距最近一次身份可信观测（相机或无歧义雷达）的时间，
并非离开相机后固定 8 秒强制丢失。未扩大关联距离、观测超时或运动限幅。

新增回归覆盖不同 odom 原点/朝向、前方杂波、相机断流后连续 12 秒后方雷达接力、
雷达真实丢失后轨迹到期，以及前方杂波仍被相机证伪。
本地完整测试：423 passed、8 skipped、6 subtests passed；尚未部署及实车验证。
若车体遮挡导致相机与雷达同时无有效观测，仍不能持续确认人的位置；本次修复不消除物理盲区。

### 车后站定后掉锁的补充修复

合成复现：已确认目标绕到车后并站定，旁边 0.65 m 有另一个小点簇。
旧逻辑每帧仍关联到目标，但只要 0.8 m 内有其他点簇就不刷新身份可信时间，
即使该点簇已被轨迹的马氏距离门排除，仍在约 8 秒后删除目标。
现在在滤波更新前，按同一观测时刻和原关联门检查附近点簇是否确实可能属于该轨迹。
不符合关联门的邻近点簇不再阻止可信时间刷新；仍有两个可行候选时保留身份超时。

回归场景覆盖车后站定 30 秒、附近无关点簇，以及两个相距 0.15 m 的可行候选仍超时退出。
这是本地合成复现，尚无现场扫描记录用于确认本次实车掉锁的具体原因，尚未部署。

补充修复后完整测试：425 passed、8 skipped、6 subtests passed。原集成测试中“旁边 0.7 m 有椅子腿就应超时”的断言已改为验证同一轨迹持续有效；真正存在两个可行候选的超时由新增回归覆盖。
