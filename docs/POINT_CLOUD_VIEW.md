...
## 后续改动：长期累积、剔除车体自身反射、性能与着色器修正

### 1. 两种累积模式（`RO2_CLOUD_HISTORY_S`）

原来只有「最近 45 秒滑动窗口」一种。走一圈回来，先看过的房间已经过期消失了，
这对「看一眼现在周围有什么」够用，但建不出一张走完全屋的三维图。

现在 `display_history_s`（环境变量 `RO2_CLOUD_HISTORY_S`）可以设成 `0`：

| 模式 | 设置 | 行为 | 代价 |
|---|---|---|---|
| 最近窗口（默认） | `RO2_CLOUD_HISTORY_S=45` | 只保留最近 45 s 的观测 | 走过的区域会消失 |
| 长期累积 | `RO2_CLOUD_HISTORY_S=0` | 观测不过期，走过的地方一直留着 | 人走动留下的残影也一直留着 |

两种模式内存都由 `display_point_limit`（默认 60000 点）兜住，超了淘汰**最久没有被重新观测到**
的格子。长期累积不等于无上限——4GB 的板子上无上限就是迟早 OOM。

网页侧栏会写明当前是哪种模式。**两种模式都只是显示层，都不能当作导航碰撞图。**

其它可调项：`RO2_CLOUD_VOXEL_M`（选样间距，默认 0.05）、`RO2_CLOUD_POINTS`（点数上限）、
`RO2_CLOUD_RADIUS_M`（入图半径，默认 20 m，原来写死 20）。

### 2. 剔除车体自身反射

俯视 15° 的 Astra 看得见自己的车头，N10P 也会打到自己的车壳。这些点如果进了地图，
会在车身周围拖出一条**跟着车一起动**的假墙——看起来特别像真障碍物。

`cloud_scene.body_mask()` 按实测车体尺寸（前 0.67 / 后 0.18 / 半宽 0.335 m）在**车体坐标系**
里判断，跟着车头转。用一个固定朝向的世界方框是不行的：车一转弯就会开始吃掉真实障碍物。

高度阈值 `body_height_m` 默认 **0.45 m**（车壳实测 0.34 m 加余量），不是「安全高度」。
设高了会把车体正上方的门梁、货架下沿一起抹掉，那些是真实几何，应该留在地图里。

需要关掉（例如排查滤波是不是吃错了东西）：`-p drop_self_hits:=false`。

### 3. 累积路径性能

`VoxelHistory` 内部从 `OrderedDict` + 逐格 Python 循环，改成按体素键排序的并行 NumPy 数组
（`np.searchsorted` 批量查重、`argpartition` 批量淘汰）。x86 实测，60000 点满载时：

| | 改之前 | 改之后 |
|---|---|---|
| `add()` 每帧 | 29.5 ms | 5.2 ms |
| `array()` 每次发布 | 55.7 ms | 0.07 ms |

RK3588 上大约是这些数字的三倍。这两个调用都在**持锁**路径上，慢一次网页和屏幕一起卡一次，
所以这不只是省电——它直接对应「点一下网页按钮要等半天」。

对外语义没有变：同一帧同一格保留第一个真实采样、跨帧后来的覆盖先前的、超上限淘汰最久未见的格子。
原有的 `test_cloud_scene.py` 全部仍然通过。

### 4. WebGL 着色器精度修正（会导致三维视图静默降级）

`cloud_viewer.js` 的顶点与片元着色器共用 `uniform int primitive`。GLSL ES 1.00 里
顶点着色器的 `int` 默认 `highp`、片元着色器默认 `mediump`，同名 uniform 精度不一致，
严格的实现（新版 Chromium、ANGLE、Mali）**直接链接失败**。

失败是静默的：`CloudView` 构造抛异常 → 页面退到软件预览，只在角落写一行「软件预览」。
在有显卡的机器上这等于白装了显卡，而且不会有人发现。

修正是在顶点着色器里显式写 `precision mediump int;`。
`tests/test_cloud_browser.py` 里补了断言：有 WebGL 时必须真的走 GL 路径，不能是 `cloudView.cpu`。

这个问题之前测不出来，是因为三项浏览器测试都要求 `chromium` 在 PATH 上，容器和 CI 里
只有 Playwright 自带的那份，于是三项全部被跳过。现在找不到系统 Chromium 会回退到自带的。

### 5. 测试

新增 `tests/test_cloud_history.py`（32 项，stdlib `unittest`，不依赖 pytest）：
体素累积与去重、每帧内存上限、TTL 窗口与长期累积两种模式、LRU 淘汰顺序、
超范围坐标不发生键碰撞、车体自身反射剔除（含跟随车头转、门梁不被误删）、
节点层的 TF 缺失/过期丢帧、状态字段、不发布任何 `cmd_vel`，以及一条性能预算断言
（退化回原来的量级会直接失败）。

`tests/ros_stubs.py` 是配套的最小 ROS 替身，**不是** ROS 集成测试：
只替换消息容器、`Node` 基类和 TF 缓存的接口形状。序列化、QoS、执行器、实车行为都不覆盖。

测试里的一切点云都标注为 **SYNTHETIC TEST DATA**，截图不代表实车建图结果。

### 没有改动的部分

`person_follower.py`、`motion_safety.py`、AEB、轮趣底盘控制、串口协议、跟随安全参数一律未动。
本次改动没有新增任何运动指令，没有「自动解除急停」「自动忽略人体障碍」「没有定位也继续跑」的逻辑，
也没有把 Nav2 接到底盘上。显示层仍然只读。
