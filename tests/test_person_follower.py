...
class FollowerHarness:
    ...
    def feed(self, people=(), scan=None, speed=0.0, yaw=0.0, voltage=25.0):
        ...
        self._cb('/scan')(scan or FakeScan())
...
class TestNodeBringUp(FollowerTestCase):
    ...
    def test_node_constructs(self):
        """曾经仓库里少了 follower_recovery.py,这一行就会 ImportError。"""
        h = self.make()
        self.assertEqual(h.node.state, 'STANDBY')

    def test_all_topics_are_subscribed(self):
        h = self.make()
        topics = {t for t, _ in h.node.subscriptions_}
        self.assertEqual(topics, {'/camera/ai_detection/targets', '/scan',
                                  '/voltage', '/wheeltec/status'})
