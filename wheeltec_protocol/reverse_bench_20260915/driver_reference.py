...
class Config:
    ...
    def __post_init__(self):
        ...
        if not 0 < self.max_speed_m_s <= 0.5 or not 0 < self.max_steering_rad < math.pi / 3:
            raise ValueError("invalid speed or steering limit")
        if not 0 < self.max_yaw_rate_rad_s <= 1.5:
            raise ValueError("invalid yaw limit")
        if not 0 < self.acceleration_m_s2 <= 1 or not 0 < self.steering_rate_rad_s <= 2:
