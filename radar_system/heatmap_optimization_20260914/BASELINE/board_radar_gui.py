    ...
    def run(self):
        ...
        def depth_callback(msg):
            ...
            try:
                ...
                # 科学优化空间深度热力图：近暖远冷（近处预警大红，远处安全冷蓝），消除生硬断层
                valid_mask = (depth_arr > 200) & (depth_arr <= 5500)
                clipped = np.clip(depth_arr, 350, 4500).astype(np.float32)
                # 距离越近 (350mm) -> 255 (高亮红/暖黄)；距离越远 (4500mm) -> 0 (深邃冷蓝)
                norm = ((4500.0 - clipped) / (4500.0 - 350.0) * 255.0).astype(np.uint8)
                # 轻量 3x3 中值滤波消除结构光斑点噪点
                norm_smooth = cv2.medianBlur(norm, 3)
                colored = cv2.applyColorMap(norm_smooth, cv2.COLORMAP_TURBO)
                colored[~valid_mask] = [18, 14, 24]

                colored_rgb = cv2.cvtColor(colored, cv2.COLOR_BGR2RGB)
                qimg = QImage(colored_rgb.data, msg.width, msg.height, msg.width * 3, QImage.Format_RGB888).copy()
...
class BoardRadarMainWindow(QWidget):
    ...
    def on_ai_frame(self, qimage):
        ...
        self.latest_ai_pixmap = pix.scaled(self.video_box.size(), Qt.IgnoreAspectRatio, Qt.SmoothTransformation)
    ...
    def on_rgb_frame(self, qimage):
        if self.cam_mode == 'ai' and self.last_targets:
            ...
            qimage = img
        pix = QPixmap.fromImage(qimage)
        scaled_pix = pix.scaled(self.video_box.size(), Qt.IgnoreAspectRatio, Qt.SmoothTransformation)
