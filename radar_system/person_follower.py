...
from footprint import (  # noqa: E402
    ...
    limit_steer_for_clearance, optical_to_vehicle,
)
...
class FollowerConfig:
    ...
    # ---- 避障 ----
    # 以下均为车头到障碍物的**净空**
    obstacle_standoff_m: float = 0.30   # 正常停车时车头前应保留的净空
    aeb_clearance_m: float = 0.15       # 硬急停线:净空小于此值无条件发 0
    aeb_release_clearance_m: float = 0.25   # 急停解除回差
    ...
    footprint_margin_m: float = 0.06     # 侧向安全余量
    ...
    min_path_clearance_m: float = 0.35   # 低于此净空就收舵找更直的路,而不是硬停
...
class PersonFollowerNode(Node):
    ...
    def on_scan(self, msg):
        ...
        # 也不知道转弯时车体会扫到哪里。
        self.scan_points = scan_to_vehicle_frame(bearings, self.lidar_mount)
        self.scan_stamp = time.monotonic()
        # 硬急停按**车宽走廊内**的最近点判定,而不是整个扇区的最近点 ——
        # 走廊里侧过一堵墙不该触发急停,但挡在轮子正前方的东西必须触发。
        corridor_near = swept_path_clearance(
            self.scan_points, self.footprint, self.cfg.geometry, 0.0)
    ...
    def control_loop(self):
        ...
        if scan_fresh:
            # 沿**实际行驶路径**检查:直行时是一条等宽走廊,转弯时是车体扫过的
            # 圆环(内侧后轮切内弯、外侧前角甩出去)。这两处才是刮门框的元凶。
            # 净空从车头最前端算起,所以包络的 stop_m 也按车头理解。
            self.path_clearance = swept_path_clearance(
                self.scan_points, self.footprint, cfg.geometry, self.cmd_steer)
            # path_clearance 已是车头净空,直接喂给以 obstacle_standoff_m 为归零点
            # 的包络。(之前误写成 clearance + standoff,归零点变成 0,贴上去才停。)
        ...
        # ---------- 按净空收舵 ----------
        # 几何上转弯需要的通道比直行更宽(见 footprint._swept_radii),
        # 所以在窄门里跟着人修方向必然刮。人往旁边偏一点,车本能地打舵去追,
        # 恰好在门框里扫出最宽的轨迹。这里先把转角往中间收到路走得通为止,
        # 收到笔直还不行才是真过不去 —— 那时净空为 0,包络自会把车停住。
        self.steer_limited = False
        if scan_fresh and abs(desired_steer) > 1e-4:
