...
from PyQt5.QtWidgets import (QApplication, QWidget, QVBoxLayout, QHBoxLayout, 
                             ...
                             QDialog, QLineEdit, QComboBox, QShortcut)
from PyQt5.QtCore import Qt, pyqtSignal, QThread, QPointF, QSize, QTimer, QRectF
from PyQt5.QtGui import QPainter, QColor, QPen, QBrush, QFont, QImage, QPixmap, QFontMetrics, QKeySequence
...
from camera_view import CameraViewport
...
class ROSThread(QThread):
    ...
    def run(self):
        ...
        node.create_subscription(String, '/camera/ai_detection/observations', observations_callback, 1)
...
class BoardRadarMainWindow(QWidget):
    def __init__(self):
        ...
        for key,mode in [('1','ai'),('2','rgb'),('3','depth')]:
            ...
            self.camera_shortcuts.append(shortcut)
    ...
    def init_ui(self):
        ...
        # 视频画面显示容器 (固定比例自适应缩放)
        self.video_box = CameraViewport(self)
        ...
        heat_label=QLabel('热图量程')
        heat_label.setStyleSheet('color:#b9cadb; font-size:12px; border:none; background:transparent; padding:0;')
        depth_tools.addWidget(heat_label)
        ...
        self.heat_range.setStyleSheet('font-size:13px; color:#dbe8f5; background:#162435; border:1px solid #385167; border-radius:6px; padding:3px 10px;')
        depth_tools.addWidget(self.heat_range)
        self.depth_center_btn=QPushButton('测点回中')
        self.depth_center_btn.setMinimumHeight(34)
        self.depth_center_btn.setStyleSheet('font-size:13px; color:#dbe8f5; background:#162435; border:1px solid #385167; border-radius:6px; padding:3px 10px;')
    ...
    def change_heat_range(self, index):
    ...
    def refresh_camera(self):
        ...
        if record is None or now-record['received']+record['age']>.8:
            ...
            self._camera_shown=None
            return
        new_frame=record is not self._camera_shown
        obs_changed=self.cam_mode=='ai' and self.ros_thread.observations is not self._camera_obs_shown
        if not new_frame and not obs_changed:return
        self._camera_shown=record
        self._camera_obs_shown=self.ros_thread.observations
        ...
        if new_frame:self._note_fps()
        else:self.cam_fps_badge.setText(self.cam_fps_badge.text().split(' · ')[0])
